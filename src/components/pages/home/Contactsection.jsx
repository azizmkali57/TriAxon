...
export default function ContactSection() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setStatus("loading");
    // Replace with your actual form submission logic (e.g., Formspree, Resend, etc.)
        ...
        <div className="lg:col-span-2 space-y-5 reveal">
          {contactInfo.map((info) => (
            <div key={info.label} className="flex items-start gap-4">
              ...
              <div>
                <div className="text-xs text-[#9CA3AF] mb-0.5">{info.label}</div>
                {info.href && info.href !== "#" ? (
                  <a href={info.href} className="text-sm text-white hover:text-[#00B4C8] transition-colors font-medium">
                    {info.value}
                  </a>
                ) : (
          ...
          >
            ...
            <div>
              ...
              <div className="text-xs text-[#9CA3AF]">Fastest response guaranteed</div>
            </div>
            <svg className="w-4 h-4 text-[#9CA3AF] ml-auto group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          ...
          {status === "success" ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-[#9CA3AF] text-sm">
                We'll get back to you within 24 hours with a detailed project plan.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  <label className="block text-xs text-[#9CA3AF] mb-1.5">Phone Number</label>
                  <input
                    type="tel"
                    name="phone"
                    value={form.phone}
                ...
                <div>
                  <label className="block text-xs text-[#9CA3AF] mb-1.5">Service Needed</label>
                  <select
                    name="service"
                    value={form.service}
                    onChange={handleChange}
                  ...
                  >
                    ...
                    <option value="web" className="bg-[#0A1628]">Web Development</option>
                    <option value="mobile" className="bg-[#0A1628]">Mobile App</option>
                    <option value="design" className="bg-[#0A1628]">UI/UX Design</option>
                    <option value="cloud" className="bg-[#0A1628]">Cloud & DevOps</option>
                    <option value="ai" className="bg-[#0A1628]">AI Integration</option>
                    <option value="enterprise" className="bg-[#0A1628]">Enterprise Software</option>
                  </select>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs text-[#9CA3AF] mb-1.5">Project Details *</label>
                <textarea
                  name="message"
                  required
                  rows={4}
              ...
              <Button type="submit" size="lg" className="w-full justify-center" loading={status === "loading"}>
                Send Message
              </Button>
            </form>
          )}
